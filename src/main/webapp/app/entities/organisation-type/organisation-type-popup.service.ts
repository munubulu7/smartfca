import { Injectable, Component } from '@angular/core';
import { Router } from '@angular/router';
import { NgbModal, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
import { OrganisationType } from './organisation-type.model';
import { OrganisationTypeService } from './organisation-type.service';

@Injectable()
export class OrganisationTypePopupService {
    private isOpen = false;
    constructor(
        private modalService: NgbModal,
        private router: Router,
        private organisationTypeService: OrganisationTypeService

    ) {}

    open(component: Component, id?: number | any): NgbModalRef {
        if (this.isOpen) {
            return;
        }
        this.isOpen = true;

        if (id) {
            this.organisationTypeService.find(id).subscribe((organisationType) => {
                this.organisationTypeModalRef(component, organisationType);
            });
        } else {
            return this.organisationTypeModalRef(component, new OrganisationType());
        }
    }

    organisationTypeModalRef(component: Component, organisationType: OrganisationType): NgbModalRef {
        const modalRef = this.modalService.open(component, { size: 'lg', backdrop: 'static'});
        modalRef.componentInstance.organisationType = organisationType;
        modalRef.result.then((result) => {
            this.router.navigate([{ outlets: { popup: null }}], { replaceUrl: true });
            this.isOpen = false;
        }, (reason) => {
            this.router.navigate([{ outlets: { popup: null }}], { replaceUrl: true });
            this.isOpen = false;
        });
        return modalRef;
    }
}
